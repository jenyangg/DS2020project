...
func receive_heartbeat(pingchan chan int, rchan chan Request, position int) {
	...
	loop:
		...
		fmt.Println("Stopped receiving heartbeats for now, finding new debugger...")
}

func debugger(timings chan map [int]time.Duration, pingchan chan int, rchan chan untypedRequest, wchans map [int]chan untypedRequest, dcchan chan untypedRequest, position int){
	clientstr := "Client " + strconv.Itoa(position) + " "
	...
	var omtx = &sync.Mutex{}
	// specifieddelay := 4000
	//go do_tasks(duration,position) client should run this
	go get_rtts(wchans,position)
	fmt.Printf("INITIALISING on node %d with wchans %d\n", position,wchans)
	initloop:
		for{
			select{
			case this_laundry_list := <- timings:
				
				personal_laundry_list = this_laundry_list
				for  k := range wchans{
					// fmt.Printf("%T,%T\n",k,v)
	...
	for{
			...
			case clientmsg := <- rchan:
				...
				} else if strings.Contains(clientmsg.a.(string),"is dead") {
					fmt.Println("DEADEADEAD")
				}
...
func client(pingchans []chan int, rchan chan Request, wchan chan Request, position int, drchan chan untypedRequest, dwchans map [int]chan untypedRequest) {
	...
	timings := make(chan map [int]time.Duration,5)
	go do_tasks(timings,position)

	debugger_id := position
	go debugger(timings, pingchans[debugger_id], drchan, dwchans,dcchan, position)
	...
	initloop:
		...
		task_count := 0
		go receive_heartbeat(pingchans[debugger_id],rchan,position)
	...
	// runloop:
		...
		for{
			...
			case rmsg:= <- rchan:
				...
				if strings.Contains(rmsg.a,"task done"){ //network flow in
			...
			default:
				if len(requested_tasks)<3{
					if timer == specifieddelay{
						...
						reqstart := time.Now()
						wchan <- Request{request,randomtask,position,reqstart,nullDuration} //network flow out
						//tell debugger about network flow out
						drchan <- untypedRequest{"count in",0,0}
...
func main() {
	...
	for j := 1; j<nClients+1; j++ {
		...
		go client(pingchans, rchan, wchan,j,drchan,dwchans)
	}
	...
	// fmt.Print(cswchans)
	timings := make(chan map [int]time.Duration,5)
	go do_tasks(timings,0)

	go debugger(timings, pingchans[0],dchans[0],cswchans,dcschan,0)
	// Init done
