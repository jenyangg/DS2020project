...
func receive_heartbeat(pingchan chan int, rchan chan Request, wchans map [int]chan untypedRequest, position int) {
	...
	// reviveloop
	go revive_this_debugger(rchan,position)
}
func revive_this_debugger(rchan chan Request,position int){
	counter := 0
	maxcounter := 50
	var nullDuration time.Duration
	for{
		...
		if counter == maxcounter{
				fmt.Println("new heartbeat detected, debugger has revived")
				rchan <- Request{"Debugger"+ strconv.Itoa(position) + " has revived", 0,position,time.Now(),nullDuration}
				break
		}
	}
}
func debugger(pingchan chan int, rchan chan untypedRequest, wchans map [int]chan untypedRequest, dcchan chan untypedRequest, otherclientchans map [int] chan Request, position int){
	...
	var responsibilities []int 
	responsibilities = append(responsibilities,position)
	dcchans := make (map [int] chan untypedRequest)
	dcchans[position] = dcchan
	//for network flow
		...
		for{
			...
			default:
				if len(rtt) == len(wchans) && len(laundrylists) == len(wchans)+1{
					...
					dcchans[position] <- untypedRequest{"ready",0,0}
					break initloop
				}	else{
					nrtts := "number of rtts: " + strconv.Itoa(len(rtt)) + " "
					nlls := "number of laundry lists: " + strconv.Itoa(len(laundrylists)) + " "
	...
	for{
			...
			case clientmsg := <- rchan:
				...
				if strings.Contains(clientmsg.a.(string),"COMPLETE"){
					...
					processed_by := clientmsg.b.(Triple).a.(int)
					tasknum := clientmsg.b.(Triple).b.(int)
					requester := clientmsg.b.(Triple).c.(int)
					...
					if notok != "" {
						...
						dcchans[requester] <- untypedRequest{notok,Pair{ctaskrtt,ctasktime},position}	
					} else{
						fmt.Println(clientmsg)
						dcchans[requester] <- untypedRequest{okmsg,Pair{rttdifference,taskdifference},position}
					}
				...
				} else if strings.Contains(clientmsg.a.(string),"debug me, my debugger is dead"){
					fmt.Println(dstr + "TAKEOVER, ",clientmsg.c.(int))
					responsibilities = append(responsibilities,clientmsg.c.(int))
					dcchans[clientmsg.c.(int)] = clientmsg.b.(chan untypedRequest) //add a channel for this debugger to talk to new client
					dcchans[clientmsg.c.(int)] <- untypedRequest{"ping new client",0,position}
					fmt.Println("TALKING TO NEW CLIENT ON CHANNEL",dcchans[clientmsg.c.(int)],dcchans)
...
func client(pingchans []chan int, rchan chan Request, wchan chan Request, position int, drchan chan untypedRequest, dwchans map [int]chan untypedRequest, otherclientchans map [int]chan Request) {
	...
	dchans := make(map [int]Pair)
	dchans[position] = Pair{drchan,dcchan}
	...
	// var nullTime time.Time
	var nullDuration time.Duration
	var current_dcchan chan untypedRequest = dcchan
	var current_drchan chan untypedRequest = drchan
		...
		for{
			...
			case rmsg:= <- rchan:
				...
				if rmsg.a == preparedstr{
					...
					initint := 0
					current_drchan <- untypedRequest{clientstr + "can talk to " +strconv.Itoa(rmsg.c), rmsg.c, position}
		...
		for{
			...
			case rmsg:= <- rchan:
				if strings.Contains(rmsg.a,"task done"){
					...
					current_drchan <- untypedRequest{taskcomplete,Triple{rmsg.c,tasknumber,position},completed_tasks[tasknumber]}
					...
					case rmsg:= <- rchan:
						if strings.Contains(rmsg.a,"reply to ping"){
							...
							if check == true{
								...
								new_debugger_write_to_client_channel := make(chan untypedRequest,10)
								current_dcchan = new_debugger_write_to_client_channel
								dwchans[most_stable_debugger] <- untypedRequest{clientstr + "debug me, my debugger is dead",new_debugger_write_to_client_channel,position}
								fmt.Println("Listening to new debugger/new debugger rchan ",current_dcchan)
								current_drchan = dwchans[most_stable_debugger]
								continue runloop // to refresh the current_dcchan variable
							} else{
				...
				} else if strings.Contains(rmsg.a,"revive"){
					fmt.Println(clientstr + "DETECTED REVIVAL of debugger " + strconv.Itoa(rmsg.c) ,rmsg.a)
					fmt.Println(clientstr + "changing channels to debugger " + strconv.Itoa(rmsg.c), current_drchan,current_dcchan)
			...
			case dmsg := <- current_dcchan :
				fmt.Println(clientstr + "RECEIVED FROM DEBUGGER", dmsg)
				if strings.Contains(dmsg.a.(string),"ping new"){
					fmt.Println(clientstr + "Confirm new handover to debugger " + strconv.Itoa(dmsg.c.(int)), dmsg)
					current_drchan = dwchans[dmsg.c.(int)]
					dchans[dmsg.c.(int)] = Pair{current_drchan,current_dcchan}
					fmt.Println(clientstr + "Confirm listening to new debugger ", current_drchan,len(dwchans))
					continue runloop
				} else{
					fmt.Println(clientstr + "Task report received from debugger "  + strconv.Itoa(dmsg.c.(int)) , dmsg)
				}
			default:
				if len(requested_tasks)<50{
					if timer == specifieddelay{
						...
						// current_drchan <- untypedRequest{"count out",0,0}
